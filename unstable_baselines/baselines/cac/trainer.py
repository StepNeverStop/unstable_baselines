...
class CACTrainer(BaseTrainer):
    def __init__(self,
                 ...
                 load_dir="",
                 **kwargs):
        super(CACTrainer, self).__init__(agent, train_env, eval_env, **kwargs)
        self.agent = agent
        self.buffer = buffer
        # hyperparameters
    ...
    def train(self):
        ...
        for env_step in trange(
                self.max_env_steps):  # if system is windows, add ascii=True to tqdm parameters to avoid powershell bugs
            self.pre_iter()
            log_infos = {}
